...
import type { AlternativeSuggestion, AudioContribution, TextContribution } from "@/types";
...
export function ModerationTable({
  ...
  audios: AudioContribution[];
  suggestions: AlternativeSuggestion[];
  names: Record<string, string>;
}) {
  const router = useRouter();

  async function patch(kind: "text" | "audio" | "suggestion", id: string, status: "approved" | "rejected") {
  ...
  return (
    <div className="mt-8 space-y-8">
      ...
      <section>
        <h2 className="mb-3 text-xl font-semibold">Audio</h2>
        <div className="space-y-3">
          {audios.map((item) => (
            <article key={item.id} className="rounded-xl border border-border bg-white p-4">
              <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                {item.region || "Hudud yo‘q"} · {item.duration}s · {formatDate(item.createdAt)}
              </p>
              <div className="mt-3 flex gap-2">
                <Button size="sm" onClick={() => void patch("audio", item.id, "approved")}>
                  Approve
                </Button>
                <Button size="sm" variant="destructive" onClick={() => void patch("audio", item.id, "rejected")}>
                  Reject
                </Button>
              </div>
            </article>
          ))}
          {audios.length === 0 ? <p className="text-sm text-muted-foreground">Audio yo‘q.</p> : null}
        </div>
      </section>
      <section>
